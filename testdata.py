...
FILE_PATH_PARS = 'C:/Users/mille/Desktop/HW/'

def get_products_urls(file_name, choice, path=FILE_PATH_PARS):
    if choice is None:
                    ...
                    for group_of_product in jshop_list_category:
                        ...
                        for i in product:
                            if f"https://gorillawear.ua{i.find('a').get('href')}" in jshop_temp_list:
                                ...
                                for_me.append(f"https://gorillawear.ua{i.find('a').get('href')}")
                                continue
                            else:
                                jshop_temp_list.append(f"https://gorillawear.ua{i.find('a').get('href')}")
...
def get_data_from_urls(path=FILE_PATH_PARS):
    with open(f'C:/Users/mille/Desktop/HW/123urls.txt') as file:
    ...
    try:
        ...
        for url in enumerate(urls_list[800:]):
            ...
            if len(chars) == 0:
                if get_attr1(url[1]) in temp:
                    ...
                    pr_data.append(
                    {
                        ...
                        'Старая цена': product_price.split('грн')[1],
                        'Наличие': 1,
            ...
            print(f'Отработала ссылка:{url[1]}  {url[0] + 1} из {urls_count}')
            # Запись в csv
        with open(f'C:/Users/mille/Desktop/HW/test_data111.csv', mode='w', newline='', encoding='utf8') as product_date:
        ...
        print(ex)
    return f"test_data.csv Успешно создан в "


if __name__ == '__main__':
    print(get_data_from_urls())
    # get_products_urls('123', choice=['all_from_site'])
